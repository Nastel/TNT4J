...
/**
 * <p>
 * Classes that implement this interface provide implementation for the {@link Tracker} logger.
 * </p>
 *
 * <p>
 ...
 * instance via {@code TrackingLogger}, create instances of {@link TrackingEvent} and use {@code tnt(...)} method to
 * report and log tracking activities and associated tracking events.
 *
 * <p>
 * A {@link Tracker} {@code #newActivity()} method should be used to create application activities
 * {@link TrackingActivity}. {@link Tracker} instance should be obtained per thread or use a helper
...
public interface Tracker extends Handle, KeyValueStats, UUIDFactory {
	...
	/**
	 * Obtains current/active {@link Source} handle associated with the current thread.
	 *
	...
	/**
	 * Obtains current {@link TrackingSelector} associated with this {@code Tracker} instance. Tracking selectors allow
	 * conditional logging based on a sev/key/value combinations
	 *
	...
	/**
	 * Obtains current {@link EventSink} associated with this {@code Tracker} instance.
	 *
	...
	/**
	 * Obtains current/active {@link TrackerConfig} configuration associated with the current tracker instance.
	 *
	...
	/**
	 * Obtains the top most active {@link TrackingActivity} instance at the top of the stack, {@link NullActivity} if
	 * none is available. Current activity is within the scope of the current thread.
	 *
	...
	/**
	 * Obtains the bottom most active {@link TrackingActivity} instance at the bottom of the stack, {@link NullActivity}
	 * if none is available. This represents the root (first) activity. Root activity is within the scope of the current
	 * thread.
	 *
	...
	/**
	 * Maintain thread context for all newly created activities. Activities are kept in the thread local context if set
	 * to true.
	 *
	 * @param flag
	 *            {@code true} to maintain thread context, {@code false} to not
	 * @return itself
	 */
	Tracker setKeepThreadContext(boolean flag);

	/**
	 * Return true if thread context for all newly created activities is enabled. Activities are kept in the thread
	 * local context if set to true.
	 *
	...
	/**
	 * Create a new application activity via {@link TrackingActivity} object instance. NOOP activity instance
	 * {@link NullActivity} is returned when {@link TrackingFilter} is set and returns false.
	 *
	 ...
	 *            activity severity level
	 * @param name
	 *            user defined logical name of the activity
	 * @return a new application activity object instance
	 * @see TrackingActivity
	 */
	...
	/**
	 * Create a new application snapshot via {@link Snapshot} object instance with default category and name as per
	 ...
	 *            property value
	 * @param valType
	 *            value type name from {@link ValueTypes} set
	 * @return a new property instance
	 * @see Property
	 * @see ValueTypes
	 */
	...
	/**
	 * Track and Trace a single application tracking activity Activities of type {@code OpType.NOOP} are ignored.
	 *
	 * @param activity
	 *            application activity to be reported
	 * @see TrackingActivity
	 */
	void tnt(TrackingActivity activity);

	/**
	 * Track and Trace a single application tracking event as a separate application activity. Events of type
	 * {@code OpType.NOOP} are ignored.
	 *
	 * @param event
	 *            application tracking event to be reported
	 * @see TrackingEvent
	 */
	 ...
	 * tracking logic into a single class. Setting the value to null disables the tracking filter check and none of
	 * newly created activities and events are filtered out.
	 *
	 * @param filter
	 *            tracking filter to register
	 * @see TrackingFilter
	 */
	 ...
	 * event signature using newUUID() call NOOP event instance {@code NullEvent} is returned when
	 * {@link TrackingFilter} is set and returns false.
	 *
	 * @param opName
	...
	/**
	 * Create a new instance of tracking event that can be timed and reported. NOOP event instance {@code NullEvent} is
	 * returned when {@link TrackingFilter} is set and returns false.
	 *
	 * @param severity
	 *            severity level
	 * @param opType
	 ...
	 * @see OpType
	 */
	TrackingEvent newEvent(OpLevel severity, OpType opType, String opName, String correlator, String tag, String msg,
			Object... args);
	 ...
	 * @see OpType
	 */
	TrackingEvent newEvent(OpLevel severity, OpType opType, String opName, String correlator, String tag, byte[] msg,
			Object... args);
