...
/**
 * This class implements a simple activity listener {@code ActivityListener} which enriches
 * activities with JVM, thread performance statistics when activity ends.
 *
...
public class DefaultActivityListener implements ActivityListener {
	 ...
	 * <code>DEFAULT_PROPERTY_CPU_TOTAL_TIME</code>.
	 */
   public void stopped(Activity activity) {
		...
		if (ctx != null && cpuTimingSupported) {
			...
			cpu.add(new Property(DEFAULT_PROPERTY_CPU_TIME, ((double) tmbean.getThreadCpuTime(ctx.ownerThread.getThreadId()) / 1000.0d), ValueTypes.VALUE_TYPE_AGE_USEC));
			cpu.add(new Property(DEFAULT_PROPERTY_TOTAL_USER_TIME, ((double) tmbean.getThreadUserTime(ctx.ownerThread.getThreadId()) / 1000.0d), ValueTypes.VALUE_TYPE_AGE_USEC));
		}
		activity.add(cpu);

		PropertySnapshot thread = new PropertySnapshot(DEFAULT_SNAPSHOT_CATEGORY, SNAPSHOT_THREAD, activity.getSeverity());
		...
		if (ctx != null) {
			thread.add(new Property(DEFAULT_PROPERTY_BLOCKED_COUNT, ctx.ownerThread.getBlockedCount(), ValueTypes.VALUE_TYPE_COUNTER));
			thread.add(new Property(DEFAULT_PROPERTY_WAITED_COUNT, ctx.ownerThread.getWaitedCount(), ValueTypes.VALUE_TYPE_COUNTER));
		}
		if (ctx != null && contTimingSupported) {
			thread.add(new Property(DEFAULT_PROPERTY_BLOCKED_TIME, ctx.ownerThread.getBlockedTime() * 1000, ValueTypes.VALUE_TYPE_AGE_USEC));
			thread.add(new Property(DEFAULT_PROPERTY_WAITED_TIME, ctx.ownerThread.getWaitedTime() * 1000, ValueTypes.VALUE_TYPE_AGE_USEC));
		}
		activity.add(thread);
		...
		if (ctx != null && ctx.startCPUTime > 0) {
			PropertySnapshot snapshot = new PropertySnapshot(DEFAULT_SNAPSHOT_CATEGORY, SNAPSHOT_ACTIVITY, activity.getSeverity());
			...
			if (contTimingSupported) {
				snapshot.add(new Property(DEFAULT_PROPERTY_BLOCKED_TIME, ((ctx.stopBlockTime - ctx.startBlockTime) * 1000), ValueTypes.VALUE_TYPE_AGE_USEC));
				snapshot.add(new Property(DEFAULT_PROPERTY_WAITED_TIME, ((ctx.stopWaitTime - ctx.startWaitTime) * 1000), ValueTypes.VALUE_TYPE_AGE_USEC));
			}
			ctx.overHeadTimeNano += (System.nanoTime() - start);
			snapshot.add(new Property(DEFAULT_PROPERTY_OVERHEAD_TIME, ((double) ctx.overHeadTimeNano / 1000.0d), ValueTypes.VALUE_TYPE_AGE_USEC));
			activity.add(snapshot);
		}
    }
	
	/**
	 * This method returns total CPU time in nanoseconds currently used by the current thread context.
	...
	protected static long getCurrentCpuTimeNanos(ThreadContext ctx) {
		return (cpuTimingSupported && (ctx.ownerThread != null)? tmbean.getThreadCpuTime(ctx.ownerThread.getThreadId()) : -1);
	}
...
class ThreadContext {
	...
	protected ThreadContext() {
		...
		startCPUTime = DefaultActivityListener.cpuTimingSupported ? DefaultActivityListener.tmbean.getThreadCpuTime(ownerThread.getThreadId()) : 0;
